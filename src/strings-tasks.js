...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((acc, cur) => acc + cur.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  const vowels = arr.join('');
  let ans = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i])) ans += 1;
  }
  return ans;
}
...
function isPalindrome(str) {
  ...
  while (left < right) {
    while (!(str[left].toLowerCase() >= 'a' && str[left].toLowerCase() <= 'z'))
      left += 1;
    while (
      !(str[right].toLowerCase() >= 'a' && str[right].toLowerCase() <= 'z')
    )
      right -= 1;
...
function reverseWords(str) {
  const arr = str.split(' ');
  const arr2 = arr.map((s) => s.split('').reverse().join(''));
  return arr2.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if ((str[i] <= 'm' && str[i] >= 'a') || (str[i] <= 'M' && str[i] >= 'A')) {
      ans += String.fromCodePoint(str.codePointAt(i) + 13);
    } else if (
      (str[i] <= 'z' && str[i] >= 'n') ||
      (str[i] <= 'Z' && str[i] >= 'N')
    ) {
...
function getCardId(value) {
  return (
    ...
    'A234567890JQK'.indexOf(value[value.length - 2])
  );
}
